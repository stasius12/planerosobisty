...
export default {
  ...
  computed: {
    ...
    total() {
      const percentOff = this.coupon && this.coupon.code && this.coupon.percent_off ? this.coupon.percent_off : null;
      return calculateCartAmountTotal(this.cartItems, percentOff, this.shipmentDetails.priceAmount)
    },
    personalAddressLine() {
  ...
  methods: {
    async createPaymentIntent() {
      try {
        ...
        const pi_secret = await this.$axios.$post('payment-intent', {
        ...
        await this.finalizePaymentIntent(pi_secret)
      } catch (error) {
        console.error(error)
      }
    },
    async finalizePaymentIntent(pi_secret) {
      this.stripeInstance.confirmP24Payment(
        pi_secret,
        {
          payment_method: {
            p24: {
